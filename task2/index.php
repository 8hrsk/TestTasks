...
function setList($intervalsCount) { // Функция ля случайных значений исходных интервалов. Можно задать кол-во интервалов
    $result = [];
...
function isIntervalOverlap($newInterval) {                  // Проверяем наложения
    ...
    foreach ($list as $interval) {
        ...
        if (($start1 <= $start2 && $start2 <= $end1) || ($start1 <= $end2 && $end2 <= $end1)) {
            return true;                                    // Проверяем наложение с помощью сравнений
        }
    }
